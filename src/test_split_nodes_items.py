...
class TestSplitNodesItems(TestCase):
    ...
    def test_split_image_at_the_beginning(self):
        node = TextNode(
            "![image](https://i.imgur.com/zjjcJKZ.png) is at the beginning and another ![second image](https://i.imgur.com/3elNhQu.png)",
            TextType.TEXT,
        )
        new_nodes = split_nodes_image([node])
        self.assertListEqual(
    ...
    def test_split_link_at_the_beginning(self):
        node = TextNode(
            "[link](https://example.com) is at the beginning and another [second link](https://example.org)",
            TextType.TEXT,
        )
        new_nodes = split_nodes_link([node])
        self.assertListEqual(
