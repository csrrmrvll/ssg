...
def split_nodes_items(old_nodes: list[TextNode], text_type: TextType) -> list[TextNode]:
    ...
    for old_node in old_nodes:
        ...
        matches = extract_markdown_items(original_text, text_type)
        if not matches:
            new_nodes.append(old_node)
            continue
        for item_text, item_link in matches:
            sections = original_text.split(f"{opening_symbol}{item_text}]({item_link})", 1)
            if len(sections) != 2:
            ...
            original_text = second  # Update remaining_text to the second part for further splitting
        if original_text != "":
